...
export default function DeploymentsPage() {
  ...
  const refresh = async () => {
    setError(null)
    try {
      const [deploymentsRes, meRes] = await Promise.all([
            ...
            <>
              <SheetHeader className="p-6 border-b border-border/40 bg-card/20">
                ...
                <SheetDescription className="flex items-center gap-4 text-xs mt-2">
                  ...
                  <span className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5" /> {selected.finished_at ? new Date(selected.finished_at).toLocaleString() : '—'}</span>
                </SheetDescription>
              </SheetHeader>
